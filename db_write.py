...
#     teachers = {' '.join(full_name): teacher_id for teacher_id, *full_name in cursor.fetchall()}
#     cursor.execute("SELECT `room_id`, `number`, `building_ru` FROM `rooms`")
#     rooms = {' '.join(full_address): room_id for room_id, *full_address in cursor.fetchall()}
#     cursor.execute("SELECT `number`, `time` FROM `timings`")
#     timings = {time: number for number, time in cursor.fetchall()}
#     data: list = []
#     for lesson in schedule_parser.parse_semester(group_id):
#         new_data = [subjects.get(lesson['subject'], 'NULL'),
#                     teachers.get(lesson['teacher'], 'NULL'),
#                     rooms.get(lesson['room'] + ' ' + lesson['building'], 'NULL'),
#                     timings.get(lesson['time'], 'NULL'),
#                     lesson.get('type', 'NULL'),
#                     group_id
#                     ]
#         try:
#             index = [row[:3] + row[4:] for row in data].index(new_data)
#             data[index][3].extend(dates_to_list(lesson['dates'], lesson['even_odd']))
#         except ValueError:
#             new_data.insert(3, dates_to_list(lesson['dates'], lesson['even_odd']))
#             data.append(new_data)
#     sql = "INSERT INTO lessons (subject_id, teacher_id, room_id, dates, lesson_number, type_ru, group_id) VALUES (%s, %s, %s, %s, %s, %s, %s)"
#     for row in data:
#         row[3] = dumps(row[3])
#         try:
    ...
    for lesson in schedule_parser.parse_semester(group_id):
        ...
        try:
            ...
            if lesson['subject'] not in subjects.keys():
                cursor.execute("INSERT INTO `subjects` (`subject_name_ru`) VALUES (%s)", lesson['subject'])
                cursor.execute("SELECT `subject_id`, `subject_name_ru` FROM `subjects`")
                subjects = {subject_name: subject_id for subject_id, subject_name in cursor.fetchall()}
